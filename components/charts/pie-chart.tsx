...
export function PieChart({ data, size, innerRatio = 0, formatValue }: PieChartProps) {
  const total = useMemo(
    ...
    <GestureDetector gesture={pan}>
      <View style={{ width: size, height: size }}>
        <Canvas style={{ width: size, height: size }}>
          ...
          {innerR > 0 && <Circle cx={cx} cy={cy} r={innerR} color={Palette.cardBackground} />}
        </Canvas>
        {activeSlice && (
          <View pointerEvents="none" style={pieOverlay(size)}>
            <Text style={chartStyles.pieLabelName}>{activeSlice.label}</Text>
            <Text style={chartStyles.pieLabelValue}>
              {formatValue ? formatValue(activeSlice.value) : activeSlice.value}
            </Text>
            <Text style={chartStyles.pieLabelPercent}>
