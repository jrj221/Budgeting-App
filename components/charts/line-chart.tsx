...
export function LineChart({
	...
	const linePathD = useMemo(() => {
		...
		return projected.map((p, i) => `${i === 0 ? "M" : "L"} ${p.px} ${p.py}`).join(" ");
	}, [projected]);
	...
	const zeroY = niceYMin < 0 && niceYMax > 0 ? projectY(0, niceYMin, niceYMax, innerH) : null;
			...
			<View style={styles.tooltip}>
				{active ? (
					<>
						...
						<Text style={[styles.tooltipAmount, { color: scheme.text }]}>
							{formatYTooltip(active.raw.y)}
						</Text>
					</>
				) : (
			...
			<GestureDetector gesture={pan}>
				<View style={[styles.canvasWrap, { width, height }]}>
					<Svg width={width} height={height}>
						...
						{xTicks.map((tick) => {
							const xPx = PAD_LEFT + ((tick - xMin) / (xMax - xMin || 1)) * innerW;
							return (
								<Line
									key={`xgrid-${tick}`}
									x1={xPx}
					...
					{xTicks.map((tick, i) => {
						...
						const labelW = 64;
						const left = isFirst ? xPx : isLast ? xPx - labelW : xPx - labelW / 2;
						const textAlign: "left" | "right" | "center" = isFirst ? "left" : isLast ? "right" : "center";
						return (
							<Text
								key={`xlabel-${tick}`}
								numberOfLines={1}
								style={[styles.axisLabel, { bottom: 4, left, width: labelW, textAlign }]}
							>
								{formatX(tick)}
							</Text>
						);
